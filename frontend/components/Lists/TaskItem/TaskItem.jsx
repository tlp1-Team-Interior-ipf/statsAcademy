...
const TaskItem = ({ task, handleEditTasks, setTasks }) => {
    ...
    return (
        <View style={{ height: 110, width: '90%', padding: 17, backgroundColor: '#22a', borderRadius: 5, margin: 5, borderTopWidth: 6, borderColor: '#56a' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <View style={{gap: 5, width: '80%'}}>
                    <Text style={{ color: "#fff", fontWeight: 'bold' }}>Fecha: {formatDate(task.date)}</Text>
                    <Text style={{ color: "#fff" }}>Titulo: {task.title}</Text>
                    <Text style={{ color: "#fff" }}>Descripcion: {task.description}</Text>
                </View>
